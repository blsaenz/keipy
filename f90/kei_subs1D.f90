...
! **********************************************************************
! Subroutine to Coordinate computation of the expansion coefficients of
! seawater with respect to temperature (alpha), salinity (beta), and
! pressure (kappa) using the 1980 equation of state for seawater.

! Subroutines Bet80,Alf80,Kap80 are called IN THAT ORDER to assure a
! minimum of repetitive work between the subroutines, as well as to
...
! to THIS subroutine which values are desired.
...
! Since Kappa doesn't require density values to be known, but does
...
! call to BLKMOD in Sig80 won't be necessary.  There is, however, a
...
! Units used in this subroutine and external subroutines:
...
! Compute Alpha next overwriting many common variables used by
! Sig80
...
!    *,Coeff. of Haline Contraction <870330.1552>
...
! Sig80, therefore we must go to that subroutine at entry point
! BlkMod to obtain those coefficients.  Similarly, if density
! has not been computed, we go to entry BlkMod in subroutine
...
! If Entry at BlkMod Exit Now
    ...
    Rho=1000.0 + Sig
    RETURN
    end Subroutine Sig80

end module kei_subs1D
